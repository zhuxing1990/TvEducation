...
public class FlyBorderView extends View
{
	...
	private void flyWhiteBorder(float x, float y)
	{
		...
		animate().translationX(x).translationY(y).setDuration(500).setInterpolator(new DecelerateInterpolator()).start();
	...
	private void flyWhiteBorder(int width, int height, float x, float y)
	{
		...
		animate().translationX(x).translationY(y).setDuration(500).scaleX(scaleX).scaleY(scaleY).setInterpolator(new DecelerateInterpolator()).start();
	}
