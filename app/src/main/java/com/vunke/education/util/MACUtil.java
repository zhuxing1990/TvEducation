...
public class MACUtil {
	...
	public static String getLocalMacAddressFromBusybox() {
		...
		if (result.length() > 0 && result.contains("HWaddr") == true) {
			...
			WorkLog.i(TAG, "Mac:" + Mac + " Mac.length: " + Mac.length());
			 ...
			 */
			result = Mac;
			WorkLog.i(TAG, result + " result.length: " + result.length());
		}
		return result;
	}
	...
	private static String callCmd(String cmd, String filter) {
		...
		try {
			...
			while ((line = br.readLine()) != null && line.contains(filter) == false) {
				// result += line;
				WorkLog.i(TAG, "line: " + line);
			}

			result = line;
			WorkLog.i(TAG, "result: " + result);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return result;
